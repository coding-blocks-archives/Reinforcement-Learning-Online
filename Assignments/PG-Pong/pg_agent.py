...
def get_actor_model(input_shape, num_actions, learning_rate):
    ...
    # X = Conv2D(64, 8, strides=(2,2), data_format = 'channels_first',
    #            activation = 'relu')(X_inp)
    # X = Conv2D(32, 4, strides=(2,2), data_format = 'channels_first', 
    #            activation = 'relu')(X)
...
class PGAgent(object):
    def __init__(self, env, train_flag = True, num_episodes = 20000, learning_rate = 0.00025, gamma = 0.99,
                model_path = None, num_checkpoints = 10):
        self.env = env
        ...
        if(train_flag):
            self.actor_model, self.prediction = get_actor_model(self.env.observation_shape, self.num_actions, self.learning_rate)
        else:
    ...
    def train(self, render = False):
        ...
        for episode in range(self.num_episodes):
            ...
            self.actor_model.train_on_batch([states_batch, discounted_rewards], actions_batch)
            self.env.close()
            if(self.num_checkpoints != 0 and (episode % (self.num_episodes/self.num_checkpoints)) == 0):
                self.prediction.save('./saved_models/pg-{:06d}.model'.format(episode))
