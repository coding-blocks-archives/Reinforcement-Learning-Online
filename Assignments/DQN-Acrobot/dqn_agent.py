...
def get_model(input_shape, num_actions, learning_rate):
    ...
    model = Model(inputs = X_inp, outputs = X, name = 'Vanilla-DQN-model')
    print(model.summary())
    ...
    def train(self, render = False):
        ...
        for episode in range(self.num_episodes):
            ...
            while(1):
                ...
                if(episode >= self.train_start_episode):
                    ...
                    for i in range(batch_size):
                        if(done_batch[i]):
                            ...
                            targets[i][action_batch[i]] = reward_batch[i] + self.gamma * np.amax(targets_next_state[i])
                ...
                if(done or t>=1000):
                    ...
                    if(np.mean(self.all_epiosde_timestep[-10:]) < best_score):
                        ...
                        self.model.save('./saved_models/dqn-best_model.model')
            ...
            if(self.num_checkpoints != 0 and (episode % (self.num_episodes/self.num_checkpoints)) == 0):
                self.model.save('./saved_models/dqn-{:06d}.model'.format(episode))
            if(episode >= self.train_start_episode):
        ...
        if(self.model_path != None):
            ...
            print('Saving model at this path : ./saved_models/dqn-final.model')
